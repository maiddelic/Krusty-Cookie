...
public class MovieGUI {
	...
	public MovieGUI(Database db) {
		...
		ProducingPane producingPane = new ProducingPane(db);
		tabbedPane.addTab("Produce", null, producingPane,
				"Log in as a new user");

		BlockedPane searchingPane = new BlockedPane(db);
		tabbedPane.addTab("Search/block", null, searchingPane, "Search/block");
		...
		producingPane.displayMessage("Connecting to database ...");
		
		/* --- change code here --- */
		/* --- change xxx to your user name, yyy to your password --- */
		if (db.openConnection()) {
			producingPane.displayMessage("Connected to database");
		} else {
			producingPane.displayMessage("Could not connect to database");
		}
		producingPane.entryActions();
	}
